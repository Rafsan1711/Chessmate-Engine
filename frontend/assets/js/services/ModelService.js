...
class ModelService {
    ...
    // --- Opening Book Logic (FIXED) ---
    async getBookMove(game) {
        try {
            // FEN ক্লিনিং সরাসরি এখানে না করে ApiService এর ওপর ছেড়ে দিচ্ছি
            const fen = game.fen();
            
            // API কল (অটোমেটিক রি-ট্রাই সহ)
            const stats = await window.apiService.getOpeningStats(fen);
            
            if (!stats || !stats.moves) return null;

            // সেরা চালগুলো সর্ট করা (জনপ্রিয়তা অনুযায়ী)
            let moves = Object.entries(stats.moves)
                .sort(([,a], [,b]) => (b.white + b.black + b.draw) - (a.white + a.black + a.draw));
            
            if (moves.length === 0) return null;

            // Top 3 moves থেকে র্যান্ডমলি বেছে নেওয়া (যাতে গেম বোরিং না হয়)
            ...
            console.warn("Opening Book Error:", e);
            return null; 
        }
    }
    ...
    async getBestMove(game) {
        ...
        if (bookMoveSan) {
            ...
            if (matchedMove) {
                console.log("📘 Book Move:", bookMoveSan);
